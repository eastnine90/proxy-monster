...
authorization. Use `--jdbc-with-truncation-diagnostics` with `--jdbc` only
when the client needs Connector/J's truncation diagnostics; its output omits
that parameter and may issue `SHOW WARNINGS`.
