...
func TestSanitizeErrPacketKeepsErrnoAndSqlStateOnly(t *testing.T) {
	...
	if errno := binary.LittleEndian.Uint16(out[1:3]); errno != 1292 {
		t.Errorf("errno = %d, want 1292", errno)
	}
	...
	if msg := mysqlwire.ErrString(out); msg != "ER_TRUNCATED_WRONG_VALUE" {
		t.Errorf("message = %q, want the errno symbol", msg)
	}
...
func TestSanitizeErrPacketDefaultsSqlStateWhenAbsent(t *testing.T) {
	// A pre-4.1-shaped ERR (no '#'+SQLSTATE): 0xff, errno LE (0x0514 = 1300), then message.
	in := append([]byte{0xff, 0x14, 0x05}, []byte("Invalid utf8mb4 character string: 'FF706D'")...)
	out := sanitizeErrPacket(in)

	if binary.LittleEndian.Uint16(out[1:3]) != 0x0514 {
		t.Errorf("errno not preserved: %x", out[1:3])
	}
	if mysqlwire.ErrString(out) != "ER_INVALID_CHARACTER_STRING" {
		t.Errorf("message = %q, want the errno symbol", mysqlwire.ErrString(out))
	}
...
// client sanitized — errno + SQLSTATE preserved, message and any echoed value gone.
func TestRelayResultSetRedactsBackendError(t *testing.T) {
	...
	if binary.LittleEndian.Uint16(relayed[1:3]) != 1146 {
		t.Errorf("errno not preserved through relay: %x", relayed[1:3])
	}
	if mysqlwire.ErrString(relayed) != "ER_NO_SUCH_TABLE" {
		t.Errorf("relayed message = %q, want the errno symbol", mysqlwire.ErrString(relayed))
	}
...
func TestRelayStmtPrepareResponseRedactsBackendError(t *testing.T) {
	...
	if mysqlwire.ErrString(payload) != "ER_NO_SUCH_TABLE" {
		t.Errorf("relayed prepare ERR message = %q, want the errno symbol", mysqlwire.ErrString(payload))
	}
